...
while(cipher):
    direction = input(
        f"\nType '{directions[0]}' to encrypt, type '{directions[1]}' to decrypt:\n")
    if (direction not in directions):
        print(f"\nPlease write '{directions[0]}' or '{directions[1]}'")
        sleep(0.9)
        continue
    ...
    restart = input(
        "\nDo you want to run cipher again type 'y' if Yes otherwise type 'n': ").lower()[0]
    if(restart != 'y'):
        cipher = False
        exit()
